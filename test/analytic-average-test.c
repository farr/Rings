...
main() {
  ...
  /* Now check averaging for nearly zero-inclination, zero-eccentricity orbits. */
  init_body_from_elements(&b1, random_between(rng, 1e-3, 2e-3), random_between(rng, 1.0, 1.5), 
                          1e-3*random_between(rng, 0.0, 1.0), 1e-5*random_between(rng, 0.0, 180.0),
                          random_between(rng, 0.0, 2.0*M_PI), random_between(rng, 0.0, 2.0*M_PI),
                          zero, 0.0, 0.0, 0.0);
  init_body_from_elements(&b2, random_between(rng, 1e-4, 2e-4), random_between(rng, 10.0, 15.0),
                          1e-3*random_between(rng, 0.0, 1.0), 1e-5*random_between(rng, 0.0, 180.0),
                          random_between(rng, 0.0, 2.0*M_PI), random_between(rng, 0.0, 2.0*M_PI),
                          zero, 0.0, 0.0, 0.0);
